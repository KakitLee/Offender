...
public class ShortMFCC implements AudioProcessor{
    ...
    short[] audioShortBuffer;
    byte[] audioByteBuffer;
    float[] floatAudioShortBuffer;
    int shortBufferLength;
    ...
    public ShortMFCC(int samplesPerFrame, float sampleRate, int amountOfCepstrumCoef, int amountOfMelFilters, float lowerFilterFreq, float upperFilterFreq) {
    ...
    public boolean process(AudioEvent audioEvent) {
        ...
        //audioByteBuffer=audioEvent.getByteBuffer().clone();
        shortBufferLength=audioEvent.getBufferSize();
        //audioShortBuffer=new short[shortBufferLength];
        floatAudioShortBuffer=new float[shortBufferLength];
        //toFloatArray(audioByteBuffer,0,audioShortBuffer,0,shortBufferLength);
        for(int i=0;i<shortBufferLength;i++){
            floatAudioShortBuffer[i]=(float)((short)(audioFloatBuffer[i]*32767));
        }
        // Magnitude Spectrum
        float bin[] = magnitudeSpectrum(floatAudioShortBuffer);
        // get Mel Filterbank
...
//    }
    ...
    public short[] toFloatArray(byte[] in_buff, int in_offset,
                                short[] out_buff, int out_offset, int out_len) {
        int ix = in_offset;
        int ox = out_offset;
